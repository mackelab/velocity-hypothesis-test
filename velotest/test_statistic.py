...
def cos_directionality_one_cell(expression: torch.Tensor, velocity_position: torch.Tensor,
    ...
    # TODO: Just use the high dimensional velocity here
    return cos(expressions_neighbours - expression,
               (velocity_position - expression).unsqueeze(0).unsqueeze(0).expand(number_neighborhoods,
...
# TODO: Just use high dimensional velocity instead of x+v
def mean_cos_directionality_varying_neighborhoods(expression: torch.Tensor, velocity_position: torch.Tensor,
    ...
    """
        ...
        @param velocity_position: velocity of all cells
    ...
    mean_cos_neighboorhoods = torch.zeros((number_cells, number_neighborhoods))
    for i, original_index in enumerate(tqdm(original_indices_cells)):
        mean_cos_neighboorhoods[i] = torch.mean(
            cos_directionality_one_cell(expression[original_index], velocity_position[original_index],
                                        expression[neighborhoods[i]]), dim=-1)
    return mean_cos_neighboorhoods
